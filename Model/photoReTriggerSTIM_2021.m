function [newTP,trigger] = photoReTriggerSTIM_2021(TP,filename,ypos,trigger)
...
bhvfile = [filename '.bhv2'];


% check for BHV and NS6
[bhv, MLConfig, TrialRecord, ~] = mlread(bhvfile);
ns6file = [filename '.ns6'];


% extract visual stim info from BHV and ypos
Fs        = 30000; % always using NS6
if ~strcmp(MLConfig.Resolution,'1920 x 1080 120 Hz')
    error('Check your hard-coded refresh rate. Was this not run on vPixx?')
end
refreshRate = str2double(MLConfig.Resolution(13:15));
refresh   =  ceil((1/refreshRate) * Fs); % time for a screen refresh in sampels * needs hard code
% % yconv     = 0.5 + (ypos*bhv.PixelsPerDegree/bhv.ScreenYresolution); % accounts for CRT scan path * needs hardcode
...
if any(strcmp({'constant'},trigger))
    error('BMC Oct 14 2021 - Im not sure code is set up for this')
...
ns_labels  = cellfun(@(x) char(cellstr(x)),{ns_header.ElectrodesInfo.Label},'UniformOutput',0); % BM Fix, 5/24/2022. char(cellstr(x)) removes leading white space
if size(ns_labels,2) == 39 && strcmp(ns_labels{39},'ainp1')
    ns_labels{37} = ns_header.ElectrodesInfo(37).Label(1:6);
    ns_labels{38} = ns_header.ElectrodesInfo(38).Label(1:6);
    ns_labels{39} = ns_header.ElectrodesInfo(39).Label(1:6);  
end
...
%             newTP(t,s) = round((tp + adj) - refresh*yconv(t));
            newTP(t,s) = round((tp + adj) - refresh);
        end
